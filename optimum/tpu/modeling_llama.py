    ...
    def forward(
        ...
        **kwargs,
        ...
        if mesh is None:
            ...
            assert query_states.shape == torch.Size(
                (bsz, self.num_heads, q_len, self.head_dim)
            ), f"incorrect query_states shape: {query_states.shape}"
            assert key_states.shape == torch.Size(
                (bsz, self.num_heads, kv_seq_len, self.head_dim)
            ), f"incorrect key_states_states shape: {key_states.shape}"
        ...
        if mesh is None:
            ...
            ), f"incorrect atten_weight shape: {attn_weights.shape}"
            assert value_states.shape == torch.Size(
                (bsz, self.num_heads, kv_seq_len, self.head_dim)
            ), f"incorrect value_states shape: {value_states.shape}"
